...
class ImportPainterSnapshotModel {
  /// Creates an immutable snapshot of painter data with deep copies of all collections
  ImportPainterSnapshotModel({
    ...
    required this.brushColor,
        ...
        _paintPaths = paintPaths
            .map((path) => List<DrawModel?>.from(path.map((point) =>
                point != null
                    ? DrawModel(
  ...
  List<List<DrawModel?>> get paintPaths => _paintPaths
      .map((path) => List<DrawModel?>.from(path.map((point) => point != null
          ? DrawModel(
              offset: Offset(point.offset.dx, point.offset.dy),
              color: Color(point.color.value),
              strokeWidth: point.strokeWidth)
